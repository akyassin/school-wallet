...
import { signToken, hashPassword, comparePassword, verifyToken } from '@/lib/auth-server';
...
export const loginFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (!user.approved)
      throw new Error('Your account is pending approval. An administrator will review your registration.');
    if (!user.active)
